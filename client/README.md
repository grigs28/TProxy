...
脚本**每次运行都会比对证书指纹**（不是「本地有没有文件」），所以：

```bash
# 在 .18 上轮换 CA 后，只需更新发布平台的证书文件
cp /opt/TProxy/proxy/ca/tproxy-ca.crt /mnt/79-sxiad/grigs/tp/tproxy-ca.crt
# 之后各节点重跑 -c 即自动跟上（指纹不同 → 下载并重装到四处信任库）
```

**为什么不能只看「文件是否存在」**：CA 有效期到 2036 年，正常不会轮换；
**会轮换只有一个原因——私钥泄露**。那一刻需要的是「一条命令让全网节点换上新 CA」，
而不是先手工 `rm` 掉每个节点上的旧证书。这个差别在应急时才显出来。

> 服务器不可达时（取不到远端指纹）脚本会**沿用本地证书并告警**，
> 不会因为网络问题让接入失败。
